...
--- Parses an nft image from a multi-line string
--
-- @tparam string image The string containing the raw-nft-image data.
-- @treturn table The parsed image data, suitable for use with
-- @{paintutils.drawTextImage}.
function parseTextImage(image)
    expect(1, image, "string")
    local nLine = 1
    local tImage = {}
    local sMode = "text"
    local sBackground = "F"
    local sForeground = "0"
    for c in image:gmatch"." do
        if c == "\n" then
            nLine = nLine + 1
        elseif c:byte() == 30 then
            sMode = "background"
...
--- Loads an nft image from a file.
--
-- @tparam string path The file to load.
--
-- @treturn table|nil The parsed image data, suitable for use with
-- @{paintutils.drawTextImage}, or `nil` if the file does not exist.
function loadTextImage(path)
    expect(1, path, "string")
    ...
    if fs.exists(path) then
        ...
        local sContent = file:read("*a")
        file:close()
        return parseTextImage(sContent)
    end
    return nil
end

-- Draw an nft image loaded by @{paintutils.parseTextImage} or @{paintutils.loadTextImage}.
--
-- @tparam table image The parsed image data.
-- @tparam number xPos The x position to start drawing at.
-- @tparam number xPos The y position to start drawing at.
function drawTextImage(image, xPos, yPos)
    expect(1, image, "table")
    expect(2, xPos, "number")
    expect(3, yPos, "number")
