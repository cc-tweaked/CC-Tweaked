...
local function file(shell, text)
    return fs.complete(text, shell.dir(), true, false, settings.get("shell.autocomplete_hidden"))
end
...
local function dir(shell, text)
    return fs.complete(text, shell.dir(), false, true, settings.get("shell.autocomplete_hidden"))
end
...
local function dirOrFile(shell, text, previous, add_space)
    local results = fs.complete(text, shell.dir(), true, true, settings.get("shell.autocomplete_hidden"))
    if add_space then
        for n = 1, #results do
