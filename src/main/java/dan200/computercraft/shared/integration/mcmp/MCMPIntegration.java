...
public class MCMPIntegration implements IMCMPAddon {
    ...
    public static void attach(AttachCapabilitiesEvent<TileEntity> event) {
        ...
        if (tile instanceof TileAdvancedModem || tile instanceof TileWirelessModem || tile instanceof TilePeripheralBase || tile instanceof TileMonitor) {
    ...
    public void registerParts(IMultipartRegistry registry) {
        ...
        ComputerCraftAPI.registerPeripheralProvider((world, pos, side) -> {
            ...
            if (tile == null || !tile.hasCapability(MCMPCapabilities.MULTIPART_CONTAINER, null)) {
                return null;
            }
            ...
            IMultipartTile multipart = container.getPartTile(EnumFaceSlot.fromFace(side))
                                                .orElse(null);
            if (multipart == null) {
                return null;
            }
            if (multipart instanceof IPeripheral) {
                return (IPeripheral) multipart;
            }
            if (multipart instanceof IPeripheralTile) {
                return ((IPeripheralTile) multipart).getPeripheral(side);
            }

            TileEntity underlying = multipart.getTileEntity();
            if (underlying instanceof IPeripheral) {
                return (IPeripheral) underlying;
            }
            if (underlying instanceof IPeripheralTile) {
                return ((IPeripheralTile) underlying).getPeripheral(side);
            }
    ...
    private static final class BasicMultipart implements ICapabilityProvider {
        ...
        public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable Direction facing) {
            if (capability == MCMPCapabilities.MULTIPART_TILE) {
                ...
                if (wrapped == null) {
                    wrapped = this.wrapped = IMultipartTile.wrap(this.tile);
                }
