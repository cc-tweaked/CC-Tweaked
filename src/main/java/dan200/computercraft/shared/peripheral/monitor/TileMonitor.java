...
public class TileMonitor extends TileGeneric implements IPeripheralTile {
    public static final NamedBlockEntityType<TileMonitor> FACTORY_NORMAL = NamedBlockEntityType.create(new Identifier(ComputerCraft.MOD_ID,
                                                                                                                      "monitor_normal"),
                                                                                                       f -> new TileMonitor(f, false));

    public static final NamedBlockEntityType<TileMonitor> FACTORY_ADVANCED = NamedBlockEntityType.create(new Identifier(ComputerCraft.MOD_ID,
                                                                                                                        "monitor_advanced"),
    ...
    public boolean onActivate(PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!player.isSneaking() && this.getFront() == hit.getSide()) {
            if (!this.getWorld().isClient) {
    ...
    */
    ...
    public void fromTag(CompoundTag tag) {
        super.fromTag(tag);
        this.m_xIndex = tag.getInt(NBT_X);
        this.m_yIndex = tag.getInt(NBT_Y);
        this.m_width = tag.getInt(NBT_WIDTH);
        this.m_height = tag.getInt(NBT_HEIGHT);
    }

    @Override
    public IPeripheral getPeripheral(@Nonnull Direction side) {
        this.createServerMonitor(); // Ensure the monitor is created before doing anything else.
        if (this.m_peripheral == null) {
            this.m_peripheral = new MonitorPeripheral(this);
        }
