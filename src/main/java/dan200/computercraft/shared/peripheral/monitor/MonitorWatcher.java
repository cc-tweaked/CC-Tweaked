...
public final class MonitorWatcher
{
    ...
    public static void onWatch( ChunkWatchEvent.Watch event )
    {
        ...
        WorldChunk chunk = (WorldChunk) event.getWorld().getChunk( chunkPos.x, chunkPos.z, ChunkStatus.FULL, false );
    ...
    public static void onTick( TickEvent.ServerTickEvent event )
    {
        ...
        while( (!obeyLimit || limit > 0) && (tile = watching.poll()) != null )
        {
            ...
            if( !((ServerWorld) world).getChunkManager().threadedAnvilChunkStorage.getPlayersWatchingChunk( chunk.getPos(), false ).findAny().isPresent() )
            {
                continue;
            }
