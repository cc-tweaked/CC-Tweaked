...
public class TileCable extends TileGeneric implements IPeripheralTile {
    public static final NamedBlockEntityType<TileCable> FACTORY = NamedBlockEntityType.create(new Identifier(ComputerCraft.MOD_ID, "cable"),
                                                                                              TileCable::new);
    ...
    public boolean onActivate(PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!this.canAttachPeripheral() || player.isSneaking()) {
            return false;
        }
    ...
    */
    ...
    public void onNeighbourChange(@Nonnull BlockPos neighbour) {
        ...
        if (neighbour.equals(this.getPos().offset(dir)) && this.hasModem() && !this.getCachedState().canPlaceAt(this.world, this.getPos())) {
            if (this.hasCable()) {
                // Drop the modem and convert to cable
                Block.dropStack(this.getWorld(), this.getPos(), new ItemStack(ComputerCraft.Items.wiredModem));
    ...
    public Direction getDirection() {
        ...
        Direction facing = state.get(BlockCable.MODEM)
                                .getFacing();
    ...
    public void onNeighbourTileEntityChange(@Nonnull BlockPos neighbour) {
        ...
        if (!this.world.isClient && this.m_peripheralAccessAllowed) {
            ...
            if (this.getPos().offset(facing)
                    .equals(neighbour)) {
                if (this.m_peripheral.attach(this.world, this.getPos(), facing)) {
    ...
    private void updateDirection() {
        if (!this.hasModemDirection) {
            ...
            this.modemDirection = this.getDirection();
        }
    }
