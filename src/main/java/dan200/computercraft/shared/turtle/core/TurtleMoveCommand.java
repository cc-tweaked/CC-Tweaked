...
public class TurtleMoveCommand implements ITurtleCommand
{
    ...
    public TurtleCommandResult execute( @Nonnull ITurtleAccess turtle )
    {
        ...
        if( !oldWorld.method_8628( state, newPosition, VerticalEntityPosition.minValue() ) )
        {
            if( ComputerCraft.turtlesCanPush && m_direction != MoveDirection.Up && m_direction != MoveDirection.Down )
            {
                ...
                {
                    if( !entity.invalid /*&& entity.preventEntitySpawning*/ )
                    {
                        ...
                        if( entityBB != null )
                        {
                            ...
                            if( oldWorld.method_8611( null, VoxelShapes.cube( pushedBB ) ) )
                            {
                                return TurtleCommandResult.failure( "Movement obstructed" );
                            }
                        }
