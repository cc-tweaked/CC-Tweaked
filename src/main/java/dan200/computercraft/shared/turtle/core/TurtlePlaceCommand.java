...
public class TurtlePlaceCommand implements ITurtleCommand {
    ...
    private static ItemStack deployOnEntity(@Nonnull ItemStack stack, final ITurtleAccess turtle, TurtlePlayer turtlePlayer, Direction direction,
        ...
        DropConsumer.set(hitEntity, drop -> InventoryUtil.storeItems(drop, ItemStorage.wrap(turtle.getInventory()), turtle.getSelectedSlot()));
    ...
    private static ItemStack deployOnBlock(@Nonnull ItemStack stack, ITurtleAccess turtle, TurtlePlayer turtlePlayer, BlockPos position, Direction side,
        ...
        if (!World.isValid(position) || world.isAir(position) || (context.getStack()
                                                                         .getItem() instanceof BlockItem && WorldUtil.isLiquidBlock(world, position))) {
            return stack;
        }

        boolean replaceable = world.getBlockState(position)
                                   .canReplace(placementContext);
        if (!allowReplace && replaceable) {
            return stack;
        }

        if (ComputerCraft.turtlesObeyBlockProtection) {
            // Check spawn protection
            boolean editable = replaceable ? TurtlePermissions.isBlockEditable(world, position, turtlePlayer) : TurtlePermissions.isBlockEditable(world,
                                                                                                                                                  position.offset(
                                                                                                                                                      side),
                                                                                                                                                  turtlePlayer);
            if (!editable) {
