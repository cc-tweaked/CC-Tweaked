...
public class TurtleTool extends AbstractTurtleUpgrade {
    ...
    public TurtleTool(Identifier id, String adjective, Item item) {
        ...
        this.m_item = new ItemStack(item);
    }
    ...
    public Pair<BakedModel, Matrix4f> getModel(ITurtleAccess turtle, @Nonnull TurtleSide side) {
        ...
        return Pair.of(MinecraftClient.getInstance()
                                      .getItemRenderer()
                                      .getModels()
                                      .getModel(this.m_item), transform);
    }
    ...
    private TurtleCommandResult attack(final ITurtleAccess turtle, Direction direction, TurtleSide side) {
        ...
        if (hit != null) {
            ...
            if (!hitEntity.handleAttack(turtlePlayer)) {
                float damage = (float) turtlePlayer.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE)
                                                   .getValue();
    ...
    private TurtleCommandResult dig(ITurtleAccess turtle, Direction direction, TurtleSide side) {
        ...
        world.syncGlobalEvent(2001, blockPosition, Block.getRawIdFromState(state));
        ...
        if (world.removeBlock(blockPosition, false)) {
            ...
            if (turtlePlayer.isUsingEffectiveTool(state)) {
                state.getBlock()
                     .afterBreak(world, turtlePlayer, blockPosition, state, tile, this.m_item.copy());
            }
        }
