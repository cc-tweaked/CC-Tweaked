...
import static dan200.computercraft.client.gui.GuiComputer.BACKGROUND_ADVANCED;
import static dan200.computercraft.client.gui.GuiComputer.BACKGROUND_COLOUR;
import static dan200.computercraft.client.gui.GuiComputer.BACKGROUND_NORMAL;
...
public final class ItemPocketRenderer extends ItemMapLikeRenderer {
    ...
    private static void renderFrame(ComputerFamily family, int colour, int width, int height) {
        ...
        MinecraftClient.getInstance()
                       .getTextureManager()
                       .bindTextureInner(colour != -1 ? BACKGROUND_COLOUR : family == ComputerFamily.Normal ? BACKGROUND_NORMAL : BACKGROUND_ADVANCED);
    ...
    private static void renderLight(int colour, int width, int height) {
        ...
        buffer.vertex(width - LIGHT_HEIGHT * 2, height + LIGHT_HEIGHT + FRAME / 2.0f, 0.0D)
              .color(r, g, b, 1.0f)
              .next();
        buffer.vertex(width, height + LIGHT_HEIGHT + FRAME / 2.0f, 0.0D)
              .color(r, g, b, 1.0f)
              .next();
        buffer.vertex(width, height + FRAME / 2.0f, 0.0D)
              .color(r, g, b, 1.0f)
              .next();
        buffer.vertex(width - LIGHT_HEIGHT * 2, height + FRAME / 2.0f, 0.0D)
              .color(r, g, b, 1.0f)
              .next();
    ...
    private static void renderTerminal(Terminal terminal, boolean greyscale, int width, int height) {
        synchronized (terminal) {
            ...
            fontRenderer.drawString(emptyLine,
                                    MARGIN,
                                    2 * MARGIN + (termHeight - 1) * FixedWidthFontRenderer.FONT_HEIGHT,
                                    terminal.getTextColourLine(termHeight - 1),
                                    terminal.getBackgroundColourLine(termHeight - 1),
                                    MARGIN,
                                    MARGIN,
                                    greyscale,
                                    palette);
            ...
            for (int line = 0; line < termWidth; line++) {
                ...
                fontRenderer.drawString(text, MARGIN, MARGIN + line * FONT_HEIGHT, colour, backgroundColour, MARGIN, MARGIN, greyscale, palette);
            }

            // And render the cursor;
            int tx = terminal.getCursorX(), ty = terminal.getCursorY();
            if (terminal.getCursorBlink() && FrameInfo.getGlobalCursorBlink() && tx >= 0 && ty >= 0 && tx < termWidth && ty < termHeight) {
                TextBuffer cursorColour = new TextBuffer("0123456789abcdef".charAt(terminal.getTextColour()), 1);
                fontRenderer.drawString(new TextBuffer('_', 1),
                                        MARGIN + FONT_WIDTH * tx,
                                        MARGIN + FONT_HEIGHT * ty,
                                        cursorColour,
                                        null,
                                        0,
                                        0,
    ...
    private static void renderTexture(BufferBuilder builder, int x, int y, int textureX, int textureY, int width, int height, int textureWidth, int textureHeight, float r, float g, float b) {
        ...
        builder.vertex(x, y + height, 0)
               .texture(textureX * scale, (textureY + textureHeight) * scale)
               .color(r, g, b, 1.0f)
               .next();
        builder.vertex(x + width, y + height, 0)
               .texture((textureX + textureWidth) * scale, (textureY + textureHeight) * scale)
               .color(r, g, b, 1.0f)
               .next();
        builder.vertex(x + width, y, 0)
               .texture((textureX + textureWidth) * scale, textureY * scale)
               .color(r, g, b, 1.0f)
               .next();
        builder.vertex(x, y, 0)
               .texture(textureX * scale, textureY * scale)
               .color(r, g, b, 1.0f)
               .next();
    }
