...
public class FileSystem {
    ...
    private void cleanup() {
        synchronized (this.m_openFiles) {
            ...
            while ((ref = this.m_openFileQueue.poll()) != null) {
                Closeable file = this.m_openFiles.remove(ref);
                if (file != null) {
                    IoUtil.closeQuietly(file);
                }
            }
    ...
    private synchronized <T extends Closeable> FileSystemWrapper<T> openFile(@Nonnull Channel channel, @Nonnull T file) throws FileSystemException {
        synchronized (this.m_openFiles) {
            ...
            FileSystemWrapper<T> fsWrapper = new FileSystemWrapper<>(this, channelWrapper, this.m_openFileQueue);
    ...
    private MountWrapper getMount(String path) throws FileSystemException {
        ...
        Iterator<MountWrapper> it = this.m_mounts.values()
                                                 .iterator();
    ...
    private static class MountWrapper {
        ...
        public ReadableByteChannel openForRead(String path) throws FileSystemException {
            ...
            try {
                if (this.m_mount.exists(path) && !this.m_mount.isDirectory(path)) {
                    return this.m_mount.openChannelForRead(path);
                } else {
                    throw new FileSystemException("/" + path + ": No such file");
                }
            } catch (IOException e) {
        ...
        public WritableByteChannel openForWrite(String path) throws FileSystemException {
            ...
            try {
                ...
                if (this.m_mount.exists(path) && this.m_mount.isDirectory(path)) {
                    ...
                    return this.m_writableMount.openChannelForWrite(path);
                }
            } catch (AccessDeniedException e) {
                throw new FileSystemException("Access denied");
            } catch (IOException e) {
        ...
        public WritableByteChannel openForAppend(String path) throws FileSystemException {
            ...
            try {
                ...
                if (!this.m_mount.exists(path)) {
                    ...
                    return this.m_writableMount.openChannelForAppend(path);
                }
            } catch (AccessDeniedException e) {
                throw new FileSystemException("Access denied");
            } catch (IOException e) {
