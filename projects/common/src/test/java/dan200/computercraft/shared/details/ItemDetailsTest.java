...
class ItemDetailsTest {
    ...
    public void testPotionDurations() {
        assertThat(
            VanillaDetailRegistries.ITEM_STACK.getDetails(PotionContents.createItemStack(Items.POTION, Potions.LONG_NIGHT_VISION)),
            containsEntryWith("potionEffects", contains(allOf(containsEntry("name", "minecraft:night_vision"), containsEntry("duration", 480.0))))
        );

        assertThat(
            VanillaDetailRegistries.ITEM_STACK.getDetails(PotionContents.createItemStack(Items.LINGERING_POTION, Potions.LONG_NIGHT_VISION)),
            containsEntryWith("potionEffects", contains(allOf(containsEntry("name", "minecraft:night_vision"), containsEntry("duration", 120.0))))
        );

        assertThat(
            VanillaDetailRegistries.ITEM_STACK.getDetails(PotionContents.createItemStack(Items.SPLASH_POTION, Potions.LONG_NIGHT_VISION)),
            containsEntryWith("potionEffects", contains(allOf(containsEntry("name", "minecraft:night_vision"), containsEntry("duration", 480.0))))
        );

        assertThat(
            VanillaDetailRegistries.ITEM_STACK.getDetails(PotionContents.createItemStack(Items.TIPPED_ARROW, Potions.LONG_NIGHT_VISION)),
